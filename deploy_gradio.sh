...
echo "   export PYTHONPATH=/workspace/Chatterbox-Multilingual-TTS:\$PYTHONPATH"
echo "   python examples/app.py"
echo ""
echo "📝 Or run in tmux (background) - copy and paste this entire command:"
echo "   cd /workspace/Chatterbox-Multilingual-TTS && tmux new-session -d -s gradio_app bash -c 'source venv/bin/activate && export HF_HUB_ENABLE_HF_TRANSFER=1 && export PYTHONPATH=/workspace/Chatterbox-Multilingual-TTS:\$PYTHONPATH && python examples/app.py 2>&1 | tee /workspace/gradio.log'"
echo ""

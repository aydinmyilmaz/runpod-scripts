...
if tmux has-session -t "$SESSION" 2>/dev/null; then
    ...
    echo "   • POST /generate - Generate TTS audio (alias)"
    echo "   • POST /upload_reference - Upload reference audio files"
    echo "   • GET /reference_files - List reference files"
    echo "   • GET /get_reference_files - List reference files (backward compatibility)"
    echo "   • GET /get_predefined_voices - List predefined voices"
